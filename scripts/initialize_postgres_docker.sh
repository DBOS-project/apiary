...
if [[ -z "$PGCONFIG" ]]; then
    docker run -d --network host --rm --name="$NAME" --env POSTGRES_PASSWORD=dbos postgres:14.5-bullseye
else
    # Use customized config file
    docker run -d --network host --rm --name="$NAME" --env POSTGRES_PASSWORD=dbos \
